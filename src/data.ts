...
import { MajorArcanaCard, MinorArcanaCard, Spread, Suit, SuitProperties } from './types';
...
TAROT_DATA.minorArcana.forEach(rawCardData => {
  const suitEmoji = TAROT_DATA.suitProperties[rawCardData.suit]?.emoji;

  // Add the suit-specific emoji to the card data before creating the instance
  const cardDataForImpl = {
    ...rawCardData,
    faceCardEmoji: suitEmoji
  };

  const card = new MinorArcanaCardImpl(cardDataForImpl);
